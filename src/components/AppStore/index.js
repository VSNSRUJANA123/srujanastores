...
class AppStore extends Component {
  state = {tabItems: tabsList[0].tabId, search: ''}
  ...
  filterItems = () => {
    ...
    const result = appsList.filter(items => items.category === tabItems)
    return result
  }
  ...
  render() {
    const {tabItems, search} = this.state
    const resultItems = this.filterItems()
    console.log(search)
    const resultsSearchItem = resultItems.filter(items =>
      items.appName.toLowerCase().includes(search.toLowerCase()),
    )
        ...
        <ul className="tabs-item">
          {tabsList.map(items => (
            <TabItem
              ...
              isActive={items.tabId === tabItems}
            />
          ))}
        </ul>
        <ul className="tabs-item">
          {resultsSearchItem.map(items => (
            <AppItem appsList={items} key={items.appId} />
          ))}
        </ul>
      </div>
    )
